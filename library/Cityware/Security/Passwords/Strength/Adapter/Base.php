...
namespace Cityware\Security\Passwords\Strength\Adapter;

use Cityware\Security\Passwords\Strength\Adapter;
...
abstract class Base implements Adapter
{
    ...
    protected function _getClassIndices($class)
    {
        ...
        if ($class == self::CLASS_LETTER) {
            $indices = array_merge(
                    $this->_getClassIndices(self::CLASS_LOWER), $this->_getClassIndices(self::CLASS_UPPER)
            );
            sort($indices);
        } else {
    ...
    protected function _getClassCount($class)
    {
        $count = 0;
        if ($class == self::CLASS_LETTER) {
            $count = $this->_getClassCount(self::CLASS_LOWER) + $this->_getClassCount(self::CLASS_UPPER);
        } else {
