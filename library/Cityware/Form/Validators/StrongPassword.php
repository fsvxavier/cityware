...
class StrongPassword extends AbstractValidator
{
    ...
    protected $messageTemplates = array(
        self::INVALID_TOO_SHORT => "Senha muito curta! Deve ter no mínimo 8 caracteres.",
    ...
    public function isValid($value)
    {
        if (strlen($value) < 8) {
            ...
            $this->error(self::INVALID_ONE_LETTER_CAPS, "Password must include at least one CAPS!");
            return false;
        } elseif (!preg_match("#\W+#", $value)) {
            $this->error(self::INVALID_ONE_SYMBOL, "Password must include at least one symbol!");
            return false;
        }
